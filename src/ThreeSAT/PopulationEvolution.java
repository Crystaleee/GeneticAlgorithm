...
public class PopulationEvolution {
	...
	private static void mutate(Individual individual) {
		...
		int pos= (int) Math.round(Math.random()*(length-2))+1;//随机选择一个基因 1~n
		individual.setSingleGene(pos, 1-individual.getSingleGene(pos));//本来是0，变为1
	}

	//单点交叉，交叉点随机生成
	static void crossover(Population newPop,Individual father,Individual mother, int index){
		int length=father.size();
		int pos=(int) Math.round(Math.random()*(length-3))+1;//length长的基因序列，1~length-2的截断位置
